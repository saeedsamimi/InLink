...
void Captcha::updateCaptcha()
{
    ...
    if (m_deformationType == Deform_SinCurve)
    {
        ...
        m_captchaImage = QImage(fm.size(0,m_captchaText).width() + m_vmod2 * 2 + m_padding * 2, fm.height() + m_hmod2 * 2 + m_padding * 2, QImage::Format_RGB32);
    }
    ...
    if (m_drawEllipses)
    {
        ...
        {
            int x1 = m_ellipseMaxRadius / 2.0 + ((qreal) std::rand() / RAND_MAX) * (m_captchaImage.width() - m_ellipseMaxRadius);
            int y1 = m_ellipseMaxRadius / 2.0 + ((qreal) std::rand() / RAND_MAX) * (m_captchaImage.height() - m_ellipseMaxRadius);
            int rad1 = m_ellipseMinRadius + ((qreal) std::rand() / RAND_MAX) * (m_ellipseMaxRadius - m_ellipseMinRadius);
            int rad2 = m_ellipseMinRadius + ((qreal) std::rand() / RAND_MAX) * (m_ellipseMaxRadius - m_ellipseMinRadius);
    ...
    if (m_drawNoise)
    {
        for (int i = 0; i < m_noiseCount; i++)
        {
            ...
            QColor col = QColor(((qreal) std::rand() / RAND_MAX) * 255, ((qreal) std::rand() / RAND_MAX) * 255, ((qreal) std::rand() / RAND_MAX) * 255);
...
void Captcha::generateText(int noOfChars, bool includeNumbers, bool includeSymbols, bool allCapital)
{
    ...
    if (m_textGeneration == TextGeneration_Random)
    {
        ...
        for (int i = 0; i < noOfChars * 2; i++)
        {
            ...
            if (!allCapital) chars.push_back(97 + ((qreal) std::rand() / RAND_MAX) * (122 - 97));
            if (includeNumbers) chars.push_back(48 + ((qreal) std::rand() / RAND_MAX) * (57 - 48));
            if (includeSymbols) chars.push_back(33 + ((qreal) std::rand() / RAND_MAX) * (47 - 33));
        }

        for (int i = 0; i < noOfChars; i++)
        {
            text += (char)chars[(std::rand() / (qreal) RAND_MAX) * (chars.size() - 1.0)];
        }
    ...
    else if (m_textGeneration == TextGeneration_Dictionary)
    {
        ...
        m_captchaText = m_dictionary[(std::rand() / (qreal) RAND_MAX) * (m_dictionary.size() - 1.0)];
    }
    else
    {
