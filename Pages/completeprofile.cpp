...
CompleteProfile::CompleteProfile(int ID,QWidget *parent)
    : QWidget(parent),ID(ID),closeFilter(parent)
    ,jobModel(this),countryModel(this),cityModel(this),universityModel(this),companyModel(this),companyCompleter(this)
    , ui(new Ui::CompleteProfile)
{
    ...
    ui->empTypeCombobox->addItems(QStringList{
       "Apprenticeship","Traineeship","Contract employment","Casual","Seasonal employees"
    });
...
void CompleteProfile::closeEvent(QCloseEvent *event)
{
    if(!QSqlDatabase::database().commit())
        QMessageBox::critical(nullptr,"Error", "The information not saved. an error occured");
...
void CompleteProfile::on_continueBtn_clicked()
{
    if(ui->startYearSpinbox->value() > QDate::currentDate().year()){
        QMessageBox::critical(this,"Error in year selecting!","You cannot start from newer years!");
        return;
    }
    if(ui->endYearSpibbox->value() > QDate::currentDate().year()){
        QMessageBox::critical(this,"Error in year selecting!","You cannot end in newer years!");
        return;
    }
    if(ui->endYearSpibbox->value() < ui->startYearSpinbox->value()){
        QMessageBox::critical(this,"Error in year selecting!","You cannot end past when you start!");
        return;
    }
    if(ui->firstNameInput->text().isEmpty() || ui->lastNameInput->text().isEmpty()){
        QMessageBox::critical(this,"Error in input name!","Please specify your first name and last name!");
        return;
    }
...
void CompleteProfile::on_dateEdit_editingFinished()
{
    ...
    if(date > QDate::currentDate()){
        QMessageBox::critical(this,"Error in using time!","The selected time is newer than now!");
        return;
    }
    if(date.year() + 5 > QDate::currentDate().year()){
        QMessageBox::critical(this,"Error in choosing birthDate","The user must at least five years");
        return;
    }
...
{
    cityModel.setQuery(QString("SELECT city FROM cities WHERE country_id = %1").arg(countryIndex));
    universityModel.setQuery(QString("SELECT university FROM universities WHERE country_id = %1").arg(countryIndex));
    companyModel.setQuery(QString("SELECT company FROM companies WHERE country_id = %1").arg(countryIndex));
}
